    ...
    if (
        ...
        if (empty($first_name)) {
            ...
            if (mysqli_num_rows($result) > 0) {
                $error = 'Da co username nay rui';
            } else {
                $token = uniqid(); //tao ma random
                $new_pass = password_hash($pass, PASSWORD_DEFAULT); //ma hoa pass nhap vao
                $sqlinsert = "insert into account values ('$user', '$first_name', '$last_name','$email', '$new_pass',0, '$token')";

                if ($conn->query($sqlinsert) === true) {
                    // $href ="http://localhost/Lab08/source%20code/active.php?email=$email$token=$token";
                    $href = "http://localhost/active.php?email=$email&token=$token";
                    $content = "Vui long click vao link de active tai khoan <a href='$href'>Active account </a>";
                    $checkSend = sendMail($email, $first_name . ' ' . $last_name, $content);

                    $email_message = $checkSend['message'];
                    $email_status = $checkSend['status'];
                }
            }
        }
    }
            ...
            <div class="col-xl-5 col-lg-6 col-md-8 border my-5 p-4 rounded mx-3">
                ...
                <form method="post" action="" novalidate>
                    ...
                    <div class="form-group">
                        ...
                        } else if (!empty($email_message)) {
                            echo ($email_status ? "<div class='alert alert-success'>$email_message </div>" : "<div class='alert alert-danger'>$email_message </div>");
                        }
                        ?>
